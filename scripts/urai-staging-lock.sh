...
case "$GOOGLE_APPLICATION_CREDENTIALS" in
  "$RUNNER_TEMP"/*) ;;
  *) echo 'Credential path must stay under RUNNER_TEMP' >&2; exit 1 ;;
esac
[ -f "$GOOGLE_APPLICATION_CREDENTIALS" ] || { echo 'Managed staging credential file is missing' >&2; exit 1; }
node - <<'NODE'
const fs = require('node:fs');
const value = JSON.parse(fs.readFileSync(process.env.GOOGLE_APPLICATION_CREDENTIALS, 'utf8'));
if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Credential JSON must be an object');
if (value.project_id !== process.env.FIREBASE_PROJECT_ID) throw new Error('Service-account project mismatch');
if (typeof value.client_email !== 'string' || typeof value.private_key !== 'string') throw new Error('Credential is not a complete service account');
NODE

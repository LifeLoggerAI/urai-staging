...
The live Jobs candidate remains unchanged at `1515ff2bbf66f764d125eb2abe7b615c88cedb59`. Prepared receipt 013 is superseded and forensic-only. The first live preflight stopped before any Jobs mutation or push, so receipt 014 was not issued. A later planned receipt 015 was superseded before execution after the deployed narrator build inputs were independently reconstructed.

Run the guarded operator before the next standalone verifier run:
...
- proves `@google-cloud/error-reporting` is unused;
- retains and upgrades Firebase Admin for asset-worker;
- validates narrator-worker package, TypeScript and Docker build inputs through `scripts/validate-jobs-narrator-build-input.mjs`;
- proves the deployed narrator image uses only `src/**/*.ts`, runs `dist/index.js`, protects `/authz` and `/execute-job`, and contains no active Firebase Admin reference;
- removes the unused Firebase Admin declarations from narrator, spatial and studio workers only after that proof;
- merges audited transitive pins into any existing override maps instead of replacing prior controls;
...
- runs Functions and asset-worker module-load smoke, narrator build-input verification, exact-head contracts, source verification, typecheck, build, narrator output inspection, and tests;
- creates immutable receipt `URAI-WSC-20260711-JOBS-DEPENDENCY-AUDIT-016` with control SHA, audit exits, report hashes, artifact hashes, build-input proof and nonmutation declarations;
...
Build-input correction receipt `URAI-WSC-20260711-JOBS-NARRATOR-BUILD-INPUT-016` records why the legacy root `workers/narrator-worker/index.js` is not a deployed input. The Dockerfile copies only `package.json`, `tsconfig.json`, and `src`; TypeScript compiles only `src/**/*.ts`; and the image runs `dist/index.js`.

If the verifier branch, Jobs branch, filesystem boundary, registry identity, narrator build inputs, audit result, source gate, or expected changed-file set differs, the operator stops before pushing.
...
The candidate SHAs can be overridden through `ADMIN_SHA`, `PRIVACY_SHA`, and `JOBS_SHA`. Every identity must be a lowercase full 40-character SHA. The launcher proves its clean verifier checkout equals the current remote control-branch head before cleaning workspaces or running any candidate.
...
The timestamped evidence archive is written under `/tmp` and its exact path is printed at completion. When GitHub CLI is authenticated, the summary is posted to canonical control issue `LifeLoggerAI/urai-admin#46`.
